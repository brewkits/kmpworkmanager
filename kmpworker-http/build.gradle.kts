...
// HttpClientProvider.android.kt and HttpClientProvider.ios.kt hardcoded a literal
// "KmpWorkManager/2.3.4" that nobody updated across 13 subsequent releases (found during a
// senior review pass, still reading 2.3.4 at 3.3.1).
val generateLibraryVersion by tasks.registering {
    group = "build"
    description = "Generates LibraryVersion.kt so HttpClientProvider's User-Agent tracks VERSION_NAME automatically."

    val versionName = project.version.toString()
    val outputDir = layout.buildDirectory.dir("generated/kotlin/libraryVersion")
    inputs.property("versionName", versionName)
    outputs.dir(outputDir)

    doLast {
        val file = outputDir.get().asFile
            .resolve("dev/brewkits/kmpworkmanager/workers/utils/LibraryVersion.kt")
        file.parentFile.mkdirs()
        file.writeText(
            """
            // AUTO-GENERATED by the :kmpworker-http `generateLibraryVersion` task — do not edit.
            // Regenerated on every build from `project.version` (VERSION_NAME).
            package dev.brewkits.kmpworkmanager.workers.utils

            internal const val LIBRARY_VERSION = "$versionName"
...
// klib check extends it to the Kotlin/Native (iOS) API, which was unguarded: v3.5.0 added a
// public iOS pinning surface here, and without this nothing would have caught a later change
// to it. `apiCheck` was passing while `klibApiCheck` sat SKIPPED.
apiValidation {
